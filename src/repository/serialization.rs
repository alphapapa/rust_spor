...
pub (crate) fn write_anchor(anchor_path: &Path, 
                            anchor: &Anchor, 
                            repo_root: &Path) -> io::Result<()> {
    let f = File::create(anchor_path)?;
    let writer = io::BufWriter::new(f);
    
    let mut m = serde_yaml::Mapping::new();
    let file_path = anchor.file_path().strip_prefix(repo_root)
        .ok()
        .and_then(|p| p.to_str())
    ...
    m.insert(
        ...
        serde_yaml::to_value(anchor.context())
...
pub (crate) fn read_anchor(anchor_path: &Path, repo_root: &Path) -> io::Result<Anchor> {
    ...
    match serde_yaml::from_reader(reader) {
        ...
        Ok(m) => {
            ...
            let fp = m.get(&serde_yaml::Value::String("file_path".to_owned()))
            ...
            let encoding = m.get(&serde_yaml::Value::String("encoding".to_owned()))
                .and_then(|v| v.as_str())
                .ok_or(io::Error::new(io::ErrorKind::InvalidInput, 
                       "expected encoding field"))?;

            let metadata = m.get(&serde_yaml::Value::String("metadata".to_owned()))
                .ok_or(io::Error::new(io::ErrorKind::InvalidInput, 
                       "expected metadata field"))?;

            let context = m.get(&serde_yaml::Value::String("context".to_owned()))
                .ok_or(io::Error::new(io::ErrorKind::InvalidInput, 
                        "expected context field"))?;


            let context: Context = serde_yaml::from_value(context.clone())
                .map_err(|_e| io::Error::new(io::ErrorKind::InvalidInput, 
                                             "expected context field"))?;
